  ...
  void FrameInterpolator::frame2canonical(const Eigen::MatrixXd& TP, const Eigen::RowVectorXd& v, double& theta, Eigen::VectorXd& S_v)
{
 ...
 Eigen::Matrix<double, 1, 6> myExpression(
    const Eigen::Matrix<double, 2, 3> & TP,
    const double & theta,
    const Eigen::Matrix<double, 3, 1> & S_v)
{
    Eigen::Matrix<double, 2, 2> _M_0;
    _M_0 << S_v[1-1], S_v[2-1],
    S_v[2-1], S_v[3-1];
    Eigen::Matrix<double, 2, 2> _M_1;
    _M_1 << cos(theta), -sin(theta),
    sin(theta), cos(theta);
    Eigen::Matrix<double, 2, 2> M = _M_0 * _M_1;

    Eigen::Matrix<double, 2, 1> _v_0;
    _v_0 << M(1-1, 1-1), M(2-1, 1-1);
    Eigen::Matrix<double, 2, 1> _v_1;
    _v_1 << M(1-1, 2-1), M(2-1, 2-1);
    Eigen::Matrix<double, 1, 6> _v_2;
    _v_2 << _v_0.transpose() * TP, _v_1.transpose() * TP;
    Eigen::Matrix<double, 1, 6> v = _v_2;
