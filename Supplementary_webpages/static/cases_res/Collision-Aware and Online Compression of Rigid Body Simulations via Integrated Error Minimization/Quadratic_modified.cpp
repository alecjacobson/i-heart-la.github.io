...
{
    const long n = constraints.cols();
    const long m = constraints.rows();
    assert( TTT.rows() == n );
    assert( TTT.cols() == n );
    assert( constraints.rows() == m );
    assert( constraints.cols() == n );

    Eigen::MatrixXd _invLhs_0(n+m, n+m);
    _invLhs_0 << TTT, (constraints).transpose(),
    constraints, Eigen::MatrixXd::Zero(m, m);
    invLhs = _invLhs_0.inverse();
