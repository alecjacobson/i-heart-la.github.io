            ...
            for (int i = 0; i < np; i++)
                for (int ii = 0; ii < P[i].tri.size(); ii++, ti++)
                    if (!ignore[ti])
                    {
                        ...
                        Eigen::Matrix<double, 1, 3> _p0_0;
                        _p0_0 << V.at(v0)[1-1], V.at(v0)[2-1], V.at(v0)[3-1];
                        Eigen::Matrix<double, 1, 3> p0 = _p0_0;

                        Eigen::Matrix<double, 1, 3> _p1_0;
                        _p1_0 << V.at(v1)[1-1], V.at(v1)[2-1], V.at(v1)[3-1];
                        Eigen::Matrix<double, 1, 3> p1 = _p1_0;

                        Eigen::Matrix<double, 1, 3> _p2_0;
                        _p2_0 << V.at(v2)[1-1], V.at(v2)[2-1], V.at(v2)[3-1];
                        Eigen::Matrix<double, 1, 3> p2 = _p2_0;

                        double x0s = pow((p0).norm(), 2);

                        Eigen::Matrix<double, 3, 3> _Mi_0;
                        _Mi_0 << 2 * (p1 - p0),
                        2 * (p2 - p0),
                        n.transpose();
                        Eigen::Matrix<double, 3, 3> Mi = _Mi_0.inverse();

                        Eigen::Matrix<double, 3, 1> _m_0;
                        _m_0 << pow((p1).norm(), 2) - x0s, pow((p2).norm(), 2) - x0s, p0 * n;
                        Eigen::Matrix<double, 3, 1> m = Mi * _m_0;
