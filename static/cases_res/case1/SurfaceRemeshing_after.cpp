...
Point SurfaceRemeshing::minimize_squared_areas(Vertex v)
{
    ...
    Eigen::MatrixXd poly2(n, 3);
    int i = 0;
    for (auto vv : mesh_.vertices(v))
    {
        poly.row(i++) = (Eigen::Vector3d)points_[vv];
        poly2.row(i % n) = (Eigen::Vector3d)points_[vv];
    }

    // build Hessian and Jacobian
    std::vector<Eigen::Matrix<double, 3, 1>> p(n);
    for( int i=1; i<=n; i++){
        p.at(i-1) = poly.row(i-1).transpose();
    }


    std::vector<Eigen::Matrix<double, 3, 1>> q(n);
    for( int i=1; i<=n; i++){
        q.at(i-1) = poly2.row(i-1).transpose();
    }


    std::vector<Eigen::Matrix<double, 3, 1>> d(n);
    for( int i=1; i<=n; i++){
        d.at(i-1) = p.at(i-1) - q.at(i-1);
    }


    Eigen::MatrixXd _sum_0 = Eigen::MatrixXd::Zero(3, 3);
    for(int i=1; i<=d.size(); i++){
        Eigen::Matrix<double, 3, 3> _H_0;
        _H_0 << pow(d.at(i-1)(2-1), 2) + pow(d.at(i-1)(3-1), 2), -d.at(i-1)(1-1) * d.at(i-1)(2-1), -d.at(i-1)(1-1) * d.at(i-1)(3-1),
        -d.at(i-1)(1-1) * d.at(i-1)(2-1), pow(d.at(i-1)(1-1), 2) + pow(d.at(i-1)(3-1), 2), -d.at(i-1)(2-1) * d.at(i-1)(3-1),
        -d.at(i-1)(1-1) * d.at(i-1)(3-1), -d.at(i-1)(2-1) * d.at(i-1)(3-1), pow(d.at(i-1)(1-1), 2) + pow(d.at(i-1)(2-1), 2);
        _sum_0 += 1 / double((d.at(i-1)).lpNorm<2>()) * _H_0;
    }
    Eigen::Matrix<double, 3, 3> H = _sum_0;

    Eigen::MatrixXd _sum_1 = Eigen::MatrixXd::Zero(3, 1);
    for(int i=1; i<=d.size(); i++){
        Eigen::Matrix<double, 3, 1> _J_0;
        _J_0 << -d.at(i-1)(2-1) * p.at(i-1)(2-1) * q.at(i-1)(1-1) - d.at(i-1)(3-1) * p.at(i-1)(3-1) * q.at(i-1)(1-1) + d.at(i-1)(2-1) * p.at(i-1)(1-1) * q.at(i-1)(2-1) + d.at(i-1)(3-1) * p.at(i-1)(1-1) * q.at(i-1)(3-1),
        d.at(i-1)(1-1) * p.at(i-1)(2-1) * q.at(i-1)(1-1) - d.at(i-1)(1-1) * p.at(i-1)(1-1) * q.at(i-1)(2-1) - d.at(i-1)(3-1) * p.at(i-1)(3-1) * q.at(i-1)(2-1) + d.at(i-1)(3-1) * p.at(i-1)(2-1) * q.at(i-1)(3-1),
        d.at(i-1)(1-1) * p.at(i-1)(3-1) * q.at(i-1)(1-1) + d.at(i-1)(2-1) * p.at(i-1)(3-1) * q.at(i-1)(2-1) - d.at(i-1)(1-1) * p.at(i-1)(1-1) * q.at(i-1)(3-1) - d.at(i-1)(2-1) * p.at(i-1)(2-1) * q.at(i-1)(3-1);
        _sum_1 += 1 / double((d.at(i-1)).lpNorm<2>()) * _J_0;
    }
    Eigen::Matrix<double, 3, 1> J = _sum_1;
