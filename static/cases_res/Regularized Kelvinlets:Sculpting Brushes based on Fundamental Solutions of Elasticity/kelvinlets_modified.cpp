    ...
    const double & ε)
{
    ...
    Eigen::VectorXd p_ε_left_parenthesis_r_right_parenthesis = (2 * b - a) / double(pow(r_ε, 3)) * F * r - 3 / double((2 * pow(r_ε, 5))) * (2 * b * ((double)(r.transpose() * F * r)) * Eigen::MatrixXd::Identity(n, n) + a * pow(ε, 2) * F) * r;

    return p_ε_left_parenthesis_r_right_parenthesis;
}
  ...
  switch (kp.brushType) {
    ...
    case igl::BrushType::PINCH: {
      ...
      kelvinlet = [&r, &F, &r_norm_sq, &kp](const Scalar& epsilon) {
        ...
        return myExpression(F, r, r_epsilon, a, b, epsilon);
      };
      break;
    }
  }
